...
//    ViewTreeObserver vto = gestureLayout.getViewTreeObserver();
  ...
  private void enableDisableButtons() {
    ...
    try {
      InputStream inputStream = getActivity().getAssets().open("trained_labels.txt");
  ...
  private void classifyFrame() {
    ...
    char letter;
    try {
      textView = (TextView) View_Holder_Chat_Input.getChat_view().findViewById(R.id.Chat_Input_Text);
    } catch (Exception e) {
        textView = null;
    }
      ...
      count = (count+1)%3;
      alphabet[count] = "";
      letter = textToShow.charAt(0);
      alphabet[count] = alphabet[count] + letter;
      if (letter == 'n') {
        alphabet[count] = textToShow.toString().substring(0, 7);
        if (!alphabet[count].equals("nothing")) {
          alphabet[count] = "n";
        }
      }


        if(alphabet[0].equals(alphabet[1]) && alphabet[1].equals(alphabet[2])) {
          if(!alphabet[count].equals("nothing")) {
            try {
              textView.setText(textView.getText().toString().trim() + alphabet[count]);
              alphabet[0] = alphabet[1] = alphabet[2] = "";
            } catch (Exception e) {}
          }
        }
